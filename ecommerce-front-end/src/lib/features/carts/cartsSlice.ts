...
export const cartsSlice = createSlice({
  ...
  reducers: {
    ...
    applyCoupon: (state, action: PayloadAction<number>) => {
      state.adjustedTotalPrice = Math.max(0, state.totalPrice - action.payload);
    },
  },
});

export const { addToCart, removeCartItem, remove, clearCart, applyCoupon } = cartsSlice.actions;
