...
export default async function Home() {
  let newArrivals = newArrivalsData;
  let topSelling = topSellingData;
  ...
  try {
    ...
    if (apparelProducts.length > 0) {
      ...
      if (na.length > 0) newArrivals = na;

      const ts = apparelProducts.filter((p) =>
        ["C005", "C006", "C007", "C008"].includes(p.sku || "")
      );
      if (ts.length > 0) topSelling = ts;

      const targetProduct = na[0] || apparelProducts[0];
      if (targetProduct) {
